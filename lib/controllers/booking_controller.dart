...
class TutorBookingsController {
  ...
  try {
    ...
    for (var data in response) {
      ...
      try {
        ...
        if (status == 'active' && acceptedAt != null) {
          ...
          final weeksPassed = DateTime.now().difference(acceptedAt).inDays ~/ 7;
    ...
    if (completedBookings.isNotEmpty) {
      final completedBookingIds = completedBookings.map((b) => b.bookingId).toList();
      print('[DEBUG] Attempting to delete booked_slots for: $completedBookingIds');
      ...
      try {
        ...
        print('[DEBUG] Verified ${bookingsCheck.length} bookings exist in bookings table');

        // Debug 2: Check booked_slots table structure
        final slotsStructure = await supabase
            .from('booked_slots')
            .select('*')
            .limit(1);
        print('[DEBUG] booked_slots table structure sample: ${slotsStructure.isNotEmpty ? slotsStructure[0].keys.join(', ') : 'empty'}');
        ...
        print('[DEBUG] Found ${matchingSlots.length} matching booked_slots records');
        print('[DEBUG] Sample matching record: ${matchingSlots.isNotEmpty ? matchingSlots[0] : 'none'}');
        ...
        if (matchingSlots.isNotEmpty) {
          ...
          if (remainingSlots.isEmpty) {
            ...
            print('[WARNING] ${remainingSlots.length} records remain after deletion');
          }
        } else {
          ...
          print('[DEBUG] ${bookingsWithSlots.length} completed bookings have time_slots but no booked_slots');
        }
      } catch (e) {
    ...
    final allBookings = [...pendingBookings, ...activeBookings, ...completedBookings];
    for (var booking in allBookings) {
      try {
        await fetchStudentInfo(booking);
        await fetchPackageInfo(booking);
      } catch (e) {
        print('[ERROR] Failed to fetch info for booking ${booking.bookingId}: $e');
      }
    }
